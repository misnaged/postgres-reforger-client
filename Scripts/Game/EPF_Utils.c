class EPF_Utils
{
	...
	/* OVERRIDED due to dedicated server issues
	...
	*/
	protected ref PG_HttpElem m_PG_HttpElem;
	ref PG_UuidCallback m_PG_UuidCallback; 
	string PlayerUID;
	

	string CharName(int playerId)
	{
		PG_Gamemode gm = PG_Gamemode.Cast(GetGame().GetGameMode());	
		return GetGame().GetPlayerManager().GetPlayerName(playerId);
	}
	
	string GetUuidFromServer(string data)
	{
		if (!m_PG_UuidCallback)
		m_PG_UuidCallback = new PG_UuidCallback(); 
		return m_PG_UuidCallback.GetUuidFromJSON(data);
	}
	
	void Check()
	{
		IEntity player = GetGame().GetPlayerController().GetControlledEntity();
		PlayerUID = GetUuidFromServer(m_PG_HttpElem.Data);	
			//PrintFormat("PlayerUID is %1", PlayerUID);

		if (!m_PG_HttpElem.IsTriggered)
		{
			GetGame().GetCallqueue().Remove(Check);
		}
		
	}
	void PerformGETRequest(int playerId)
	{
		if (!m_PG_HttpElem)
			m_PG_HttpElem = new PG_HttpElem();
		
		 
		m_PG_HttpElem.Repeat(CharName(playerId));
		GetGame().GetCallqueue().CallLater(Check, 1000, m_PG_HttpElem.IsTriggered);
